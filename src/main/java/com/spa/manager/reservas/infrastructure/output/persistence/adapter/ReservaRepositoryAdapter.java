...
public class ReservaRepositoryAdapter implements ReservaRepositoryPort {
    ...
    @Override
    public boolean existeConflictoExcluyendo(Integer idTerapeuta, LocalDate fecha,
                                             LocalTime horaInicio, LocalTime horaFin,
                                             Integer idReservaExcluida) {
        return jpaRepository.existeConflictoExcluyendo(idTerapeuta, fecha, horaInicio, horaFin, idReservaExcluida);
    }

    @Override
    public List<Reserva> findConFiltros(Integer idTerapeuta, EstadoReserva estado,
                                        LocalDate fechaInicio, LocalDate fechaFin) {
        return jpaRepository.findConFiltros(idTerapeuta, estado, fechaInicio, fechaFin)
                .stream()
                .map(mapper::toDomain)
                .collect(Collectors.toList());
    }
    ...
    @Override
    public List<Reserva> findFinalizadasSinFactura() {
        return jpaRepository.findFinalizadasSinFactura().stream()
                .map(mapper::toDomain)
                .collect(Collectors.toList());
    }

    @Override
    public List<Reserva> findByIdTerapeutaAndFecha(Integer idTerapeuta, LocalDate fecha) {
        return jpaRepository.findByIdTerapeutaAndFecha(idTerapeuta, fecha)
                .stream().map(mapper::toDomain).collect(Collectors.toList());
    }

    @Override
    public List<Reserva> findByIdTerapeutaAndFechaBetween(Integer idTerapeuta,
                                                          LocalDate fechaInicio,
                                                          LocalDate fechaFin) {
        return jpaRepository.findByIdTerapeutaAndFechaBetween(idTerapeuta, fechaInicio, fechaFin)
                .stream().map(mapper::toDomain).collect(Collectors.toList());
    }

    @Override
    public List<Reserva> findByFechaBetween(LocalDate fechaInicio, LocalDate fechaFin) {
        return jpaRepository.findByFechaBetween(fechaInicio, fechaFin)
                .stream().map(mapper::toDomain).collect(Collectors.toList());
    }
}